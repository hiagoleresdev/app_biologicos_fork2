  ...
  ngAfterViewInit(): void {
    ...
    this.settings = of({
      show_action: {
        ...
        consulta_placeholder: 'Faça a busca por paciente, registro hospitalar, medicamento ou período de administração'
      }
    });
    const _lresult = [];
    this.service.datas.filter(d => d.dispensacao && !d.administracao).forEach((data) => {
      const _v: ControlBuilder[] = [
        ...
        },
        {
          ...
          favorite: '6', name: 'medicine', value: `Biologico ${data.medicine}`,
          validators: [], disabled: true, hideRequired: true
        },
        {
